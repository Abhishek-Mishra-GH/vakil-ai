...
const XrayPdfCanvas = dynamic(() => import("@/components/XrayPdfCanvas"), {
  ssr: false,
  loading: () => <p className="xray-canvas-note" style={{padding: 20}}>Loading PDF...</p>,
});
...
export default function XRayPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const queryDoc = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("doc") : null;
        const nextDoc = queryDoc || docData.documents.find((item) => item.processing_status === "ready")?.id || docData.documents[0]?.id || "";
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const session = await createQaSession({ case_id: selectedCaseId, document_id: selectedDocId });
        setQaSessionId(session.session_id);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Could not load x-ray insights");
      }
    })();
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!active) return;
        setPdfUrl("");
        setError(err instanceof Error ? err.message : "Could not load PDF file");
      } finally {
  ...
  const selectedInsight = useMemo(() => insights.find((item) => item.id === selectedInsightId) || null, [insights, selectedInsightId]);
  const selectedCase = useMemo(() => cases.find((item) => item.id === selectedCaseId) || null, [cases, selectedCaseId]);
  const selectedDocument = useMemo(() => documents.find((item) => item.id === selectedDocId) || null, [documents, selectedDocId]);
  
  const grouped = useMemo(() => {
    const output: Record<string, Insight[]> = { HIGH_RISK: [], MEDIUM_RISK: [], STANDARD: [] };
    insights.forEach((item) => output[item.anomaly_flag]?.push(item));
    return output;
  }, [insights]);

  const visibleOverlays = insights.filter((item) => item.page_number === currentPage);
  ...
  async function askQuestion(event: React.FormEvent) {
    ...
    const tempUserMsg: QaMessage = { role: "user", content: questionText, created_at: new Date().toISOString() };
    setQaMessages(prev => [...prev, tempUserMsg]);

    try {
      const result = await askQaSession(qaSessionId, questionText);
      const tempAiMsg: QaMessage = { role: "assistant", content: result.answer, created_at: new Date().toISOString() };
    ...
    <div className="xray-light-wrapper">
      {portalNode && createPortal(
        <div className="xray-controls-row" style={{ marginBottom: 0 }}>
          <select value={selectedCaseId} onChange={(e) => setSelectedCaseId(e.target.value)} className="xray-light-select">
          ...
          <select value={selectedDocId} onChange={(e) => setSelectedDocId(e.target.value)} className="xray-light-select">
      ...
      <div className="xray-light-grid">
        <div className="xray-light-pdf-pane">
          <div className="xray-light-pdf-controls" suppressHydrationWarning>
            <button suppressHydrationWarning onClick={() => setCurrentPage((prev) => Math.max(1, prev - 1))} disabled={currentPage <= 1}>
              Prev
            </button>
            <select value={currentPage} onChange={(e) => setCurrentPage(Number(e.target.value))} className="xray-light-select" style={{ width: 140 }}>
            ...
            <button
              ...
              onClick={() => setCurrentPage((prev) => Math.min(pageNumbers[pageNumbers.length - 1] || prev + 1, prev + 1))}
              disabled={pageNumbers.length > 0 && currentPage >= pageNumbers[pageNumbers.length - 1]}
            >
              Next
            </button>
            <div style={{width: 1, height: 20, background: "#d1d5db", margin: "0 8px"}} />
            <button onClick={() => setZoom(z => Math.max(0.5, z - 0.2))} disabled={zoom <= 0.5}>-</button>
            <span style={{fontSize: "0.8rem", width: "40px", textAlign: "center"}}>{Math.round(zoom * 100)}%</span>
            <button onClick={() => setZoom(z => Math.min(2.5, z + 0.2))} disabled={zoom >= 2.5}>+</button>
          </div>
          <div className="xray-light-pdf-canvas">
            {pdfUrl ? (
              ...
              <p style={{marginTop: 40}}>{pdfLoading ? "Loading PDF..." : "PDF unavailable"}</p>
            )}
          </div>
        </div>
        ...
        <div className="xray-light-clauses">
          <div className="ci-header">
            ...
            <div className="ci-filter-tabs">
              {(["ALL", "HIGH_RISK", "MEDIUM_RISK", "STANDARD"] as const).map((level) => (
                <button
                  key={level}
            ...
            <span className="ci-count">{filteredInsights.length} CLAUSES FOUND</span>
          </div>
          <div className="xray-light-clauses-content">
            {filteredInsights.length === 0 && (
              <p style={{ color: "#6b7280", padding: "20px", textAlign: "center", fontSize: "0.85rem" }}>No clauses found for this filter.</p>
            )}
            {filteredInsights.map((insight) => {
              const riskClass = insight.anomaly_flag === "HIGH_RISK" ? "high-risk" : insight.anomaly_flag === "MEDIUM_RISK" ? "medium-risk" : "standard";
              const riskIcon = insight.anomaly_flag === "HIGH_RISK" ? "⚠" : insight.anomaly_flag === "MEDIUM_RISK" ? "ⓘ" : "✓";
              return (
                <div
                  key={insight.id}
                ...
                >
                  <div className="ci-card-top">
                    <span className={`ci-card-label ci-card-label--${riskClass}`}>
                      {insight.clause_type} <span style={{opacity: 0.5, margin: "0 6px"}}>•</span> {insight.anomaly_flag.replace("_", " ")}
                    </span>
                    <span className={`ci-card-icon ci-card-icon--${riskClass}`}>{riskIcon}</span>
                  </div>
                  <div className="ci-card-summary">
                    {insight.summary}
                  </div>
                  {insight.anomaly_reason && (
                    <div className={`ci-card-banner ci-card-banner--${riskClass}`}>
                  ...
                  <div className="ci-card-footer">
                    <span className="ci-card-statute">
                      {insight.statutory_reference && insight.statutory_reference.toUpperCase() !== "N/A" ? `§ ${insight.statutory_reference}` : `PAGE ${insight.page_number}`}
                    </span>
                    <button 
                      ...
                      onClick={(e) => { 
                        ...
                        if (insight.anomaly_flag !== "HIGH_RISK") {
                          handleViewPrecedents(insight.summary || insight.clause_type);
                        }
                      }}
                    >
                      {insight.anomaly_flag === "HIGH_RISK" ? "DRAFT REBUTTAL" : "VIEW PRECEDENTS"}
                    </button>
                  </div>
                </div>
              );
          ...
          <div className="xray-light-chat-messages">
            {qaMessages.map((msg, i) => (
              <div key={i} className="xray-light-msg">
                <div className={`xray-light-msg-avatar ${msg.role === 'assistant' ? 'ai' : 'user'}`}>
                  <div className="xray-light-avatar-icon">
                    {msg.role === 'assistant' ? 'AI' : 'U'}
                  </div>
                  {msg.role === 'assistant' ? 'AI' : 'User'}
                </div>
                <div className="xray-light-msg-content">
                  {msg.content}
                </div>
              </div>
            ))}
          </div>
          <form className="xray-light-chat-input" onSubmit={askQuestion} suppressHydrationWarning>
            <input 
            ...
            <button type="submit" suppressHydrationWarning disabled={busy || !qaSessionId || !qaQuestion.trim()}>
              {busy ? "..." : "ASK"}
            </button>
          </form>
        </div>
      ...
      {precedentsModalOpen && (
        <div className="xray-light-modal-overlay" onClick={() => setPrecedentsModalOpen(false)}>
          <div className="xray-light-modal" onClick={e => e.stopPropagation()}>
            <div className="xray-light-modal-header">
              <h3>Legal Precedents</h3>
              <button className="xray-light-modal-close" onClick={() => setPrecedentsModalOpen(false)}>✕</button>
            </div>
            <div className="xray-light-modal-content">
              <p className="precedent-issue-label">ISSUE: <span>{selectedPrecedentIssue}</span></p>
              ...
              {precedentsLoading ? (
                ...
                <p className="precedent-empty">No highly relevant precedents found.</p>
              ) : (
                <div className="precedent-list">
                  {precedentsList.map((p, i) => (
                    <div key={i} className="precedent-card">
                      ...
                      <p className="precedent-headline" dangerouslySetInnerHTML={{ __html: p.headline }} />
                      <div className="precedent-footer">
                        <a href={p.url} target="_blank" rel="noreferrer" className="precedent-link">
                          Read Full Judgment ↗
                        </a>
                      </div>
                    </div>
